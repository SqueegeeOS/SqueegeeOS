...
const checks = [
  ...
  ["036", "referral reward claims", (s) => hasTable(s, "member_referral_reward_events") && s.rlsTables.has("member_referral_reward_events") && hasColumn(s, "member_referral_rewards", "claimed_at") && constraintIncludes(s, "member_referral_rewards", "'earned'") && s.triggers.has("public.member_referral_reward_events.member_referral_reward_events_immutable") && s.functions.has("claim_member_referral_reward") && functionConfigIncludes(s, "claim_member_referral_reward", "search_path=public"), "pending deploy"],
];
...
try {
  ...
  for (const [id, label, verify, note] of checks) {
    ...
    if (!applied && note !== "optional/parked" && note !== "pending deploy") missing += 1;
    const status = applied
      ? "PASS"
      : note === "optional/parked"
        ? "PARKED"
        : note === "pending deploy"
          ? "PENDING"
          : "MISSING";
