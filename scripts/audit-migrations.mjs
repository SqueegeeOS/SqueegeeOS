...
const checks = [
  ...
  ["080", "durable sales service interests", (s) => hasColumn(s, "sales_rep_leads", "service_interests") && constraintIncludes(s, "sales_rep_leads", "service_interests", "exterior_windows", "interior_windows", "screens", "cobweb_removal") && s.customerPublicPolicies === 0 && s.customerPublicPrivileges === 0],
  ["081", "idempotent public request submissions", (s) => hasColumn(s, "lead_intakes", "client_submission_id") && s.nullableColumns.has("lead_intakes.client_submission_id") && s.indexes.has("lead_intakes_request_submission_uidx") && constraintIncludes(s, "lead_intakes", "request_form", "client_submission_id") && s.customerPublicPolicies === 0 && s.customerPublicPrivileges === 0],
  ["083", "owner-approved payment rails", (s) => hasColumns(s, "enrollment_packets", "payment_rail", "manual_payment_approved_at", "manual_payment_approved_by") && hasColumns(s, "memberships", "payment_rail", "manual_payment_approved_at", "manual_payment_approved_by") && hasColumn(s, "signed_agreements", "payment_rail") && constraintIncludes(s, "memberships", "manual_cash_check", "automatic_billing_enabled") && constraintIncludes(s, "enrollment_packets", "manual_cash_check", "manual_payment_approved_by") && ["memberships_payment_rail_status_idx", "enrollment_packets_payment_rail_status_idx"].every((index) => s.indexes.has(index)) && s.customerPublicPolicies === 0 && s.customerPublicPrivileges === 0],
  ["084", "end-to-end sales service interests", (s) => hasColumn(s, "sales_rep_leads", "service_interests") && constraintIncludes(s, "sales_rep_leads", "service_interests", "solar_panels", "pressure_washing", "gutter_cleaning", "home_care_membership") && s.customerPublicPolicies === 0 && s.customerPublicPrivileges === 0],
  ["085", "legal review copy integrity", (s) => hasColumn(s, "agreement_document_versions", "review_copy_sha256") && constraintIncludes(s, "agreement_document_versions", "review_copy_sha256", "^[0-9a-f]{64}$") && s.customerPublicPolicies === 0 && s.customerPublicPrivileges === 0],
  ["086", "private Jobber territory map", (s) => hasTable(s, "jobber_territory_geocodes") && hasColumns(s, "jobber_territory_geocodes", "external_property_id", "source_address_hash", "latitude", "longitude", "geocode_status") && hasColumns(s, "jobber_visit_projections", "property_name", "property_address") && s.rlsTables.has("jobber_territory_geocodes") && s.customerPublicPolicies === 0 && s.customerPublicPrivileges === 0],
  ["089", "persistent technician referrals", (s) => hasTable(s, "homeatlas_technicians") && hasColumns(s, "lead_intakes", "referred_by_technician_key", "referred_by_technician_name", "referral_permission_confirmed_at") && s.rlsTables.has("homeatlas_technicians") && s.customerPublicPolicies === 0 && s.customerPublicPrivileges === 0],
];
