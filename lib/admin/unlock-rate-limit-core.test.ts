...
describe("admin unlock rate limiter", () => {
  ...
  it("lets a successful unlock clear an active lock", () => {
    const limiter = new InMemoryAdminUnlockRateLimiter();
    const now = 1_800_000_000_000;

    for (let attempt = 0; attempt < ADMIN_UNLOCK_MAX_FAILURES; attempt += 1) {
      limiter.record("identity", false, now + attempt);
    }
    expect(limiter.check("identity", now + 10).allowed).toBe(false);

    expect(limiter.record("identity", true, now + 11).allowed).toBe(true);
    expect(limiter.check("identity", now + 12).allowed).toBe(true);
  });
