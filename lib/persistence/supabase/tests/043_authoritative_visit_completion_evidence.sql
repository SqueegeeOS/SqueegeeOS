...
create temporary table forbidden_domain_content_fingerprints (
...
begin
  if requested_stage not in ('before', 'after') then
...
begin
  ...
  if exists (
    select 1
    from (
      select * from forbidden_domain_content_fingerprints where stage = 'before'
    ) before_state
    full join (
      select * from forbidden_domain_content_fingerprints where stage = 'after'
    ...
    raise exception 'Forbidden-domain fingerprints changed';
  end if;
end;
$$;
