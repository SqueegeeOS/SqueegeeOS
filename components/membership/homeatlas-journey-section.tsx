...
function JourneyLedger({ entries }: { entries: JourneyEntry[] }) {
  return (
    <div className="border-y border-white/[0.06] py-12 sm:py-14">
      <dl className="space-y-12 sm:space-y-14">
        {entries.map((entry) => (
          <div key={entry.label}>
            <dt className="text-[10px] font-medium uppercase tracking-[0.22em] text-white/35">
              {entry.label}
            </dt>
            <dd className="mt-3 font-serif text-[1.625rem] font-light leading-snug tracking-[-0.01em] text-white/[0.88] sm:text-3xl">
              {entry.value}
            </dd>
          </div>
        ))}
      ...
      <div className="mt-14 h-px bg-gradient-to-r from-transparent via-white/[0.05] to-transparent" />
    </div>
  );
}
...
export function HomeAtlasJourneySection({
  ...
  return (
    ...
    >
      <p className="text-[10px] font-medium uppercase tracking-[0.28em] text-accent/55">
        Your record
      </p>
      <h2
        id="homeatlas-journey-heading"
        className="mt-4 font-serif text-[1.75rem] font-light leading-[1.12] tracking-[-0.02em] text-white/[0.92] sm:text-4xl"
      >
        Your HomeAtlas Journey
      </h2>
      <p className="mt-5 max-w-md text-sm leading-[1.7] text-white/45">
