...
const menuSpring = { type: "spring" as const, damping: 32, stiffness: 260, mass: 0.85 };
      ...
      {open && (
        <>
          <motion.button
            ...
            className="fixed inset-0 z-[70] bg-black/20 backdrop-blur-md"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <motion.aside
            ...
            className="fixed inset-y-0 right-0 z-[80] flex w-[min(88vw,22rem)] flex-col border-l border-border/60 bg-background/92 px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] shadow-[-8px_0_40px_rgba(0,0,0,0.18)] backdrop-blur-2xl"
            initial={reduceMotion ? false : { x: "100%" }}
            animate={{ x: 0 }}
            exit={reduceMotion ? undefined : { x: "100%" }}
            transition={reduceMotion ? { duration: 0.01 } : menuSpring}
          >
