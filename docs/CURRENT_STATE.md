...
- Completion-evidence-qualified portal history is implemented as a separate slice stacked after PR4. A completed Jobber visit appears only when its immutable event exactly matches appointment, membership, property, visit, source, classification, projection, connection, and property-link authority. Exact completion evidence supersedes a stale scheduled snapshot by appointment ID; missing or malformed evidence cannot displace a valid scheduled visit. History visibility does not infer savings, obligation fulfillment, pricing, billing, Stripe state, Property Memory, or customer communication. The slice is repository-only and remains unmerged, undeployed, and production-unproven.
