...
export async function GET(request: Request) {
  ...
  const [versions, packets] = await Promise.all([
    supabase
      ...
      .select(
        "id, document_kind, version, status, review_copy_sha256, content_sha256, approved_at, approved_by, release_authority, counsel_review_status, counsel_reviewed_at, counsel_reviewed_by, review_notes, updated_at",
      )
      .order("document_kind")
    ...
    supabase
      ...
      .select(
        "id, presentation_id, customer_name, customer_email, status, payment_rail, signature_provider, docusign_status, signature_sent_at, signed_at, payment_link_sent_at, payment_completed_at, portal_ready_at, last_error_code, last_error_message, updated_at",
      )
